        ...
        for x,y in [(i,j-1),(i-1,j),(i+1,j),(i,j+1)]:
            ...
            if matrix[x][y] == 0:
                ...
                if node.is_spaceship_found() and node.gasoline > 0:
                    gasoline_spent = 1
                    cost = 1/2
                new_node = Node((x,y), node, list(node.visited_samples), found_sample, node.samples, node.acum_cost + cost, node.gasoline-gasoline_spent, node.is_spaceship_found())
                ...
                if node.is_spaceship_found() and node.gasoline > 0:
                    gasoline_spent = 1
                    cost = 1/2
                new_node = Node((x,y), node, list(node.visited_samples), found_sample, node.samples, node.acum_cost + cost, node.gasoline - gasoline_spent, True)
                heapq.heappush(priorityQueue, (new_node.acum_cost, new_node))
            elif matrix[x][y] == 6:
                cost = 1
                gasoline_spent = 0
